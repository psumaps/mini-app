...
import CheckableText from '../../../../shared/components/common/checkableText';
import Line from '../../../../shared/components/common/line';
...
const Modal = (
  { active, setActive }: { active: boolean; setActive: (active: boolean) => void },
  props: ModalProps,
) => {
  ...
  return (
    <div className={` ${active ? 'scale-100' : 'scale-0'}  flex items-center justify-center`}>
      <div
      ...
      >
        <div className="flex pr-4 pl-4 pb-[2.375rem]">
          <h2 className="pt-0.5 mx-auto c_textHeader dark:text-cd_main">Фильтры</h2>
          <Button
        ...
        {Object.entries(filter).map(([key, values], index) => (
          <div key={key}>
            <div className="c4 pb-4 text-c_textFilter dark:text-cd_main">{key}</div>
